...
public class TimeTableFragment extends Fragment {
    ...
    private void initData(Bundle bundle) {
        ...
        for (int i = dataLists.size() - 1; i >= 0; i--) {
            int dayOfWeek = getDateOfWeek(convertStringToDate(dataLists.get(i).getDate()));
            if (pageIndex == 0) {
                if (dayOfWeek > 4) {
                    dataLists.remove(i);
                }
        ...
        for (SectionSchedule item : dataLists) {
            int yPos = 0;
            int xPos = 0;
            int dayOfWeek = getDateOfWeek(convertStringToDate(item.getDate()));
            if (dayOfWeek <= 4) {
